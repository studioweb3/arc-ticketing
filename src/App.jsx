...
export default function App() {
    ...
    // Nouvelles variables pour l'affichage Spectateur
    const [currentEventLocation, setCurrentEventLocation] = useState("");
    const [currentEventPhone, setCurrentEventPhone] = useState("");
        ...
        if (targetEvent && provider && userAddress) {
            try {
                ...
                if (!isBackgroundUpdate) {
                    ...
                    // --- AJOUT : Récupérer Location et Phone pour le Spectateur ---
                    const loc = await contract.eventLocation();
                    setCurrentEventLocation(loc);
                    
                    const ph = await contract.eventPhone();
                    setCurrentEventPhone(ph);
                }
                            ...
                            <div className="w-full max-w-3xl mb-6 rounded-2xl overflow-hidden shadow-2xl border border-slate-800 relative bg-slate-900 group">
                                ...
                                <div className="absolute bottom-0 left-0 w-full p-6 z-20 flex justify-between items-end">
                                    <div>
                                        ...
                                        <p className="text-slate-300 text-xs mb-1">📍 {currentEventLocation || t("Localisation non spécifiée", "Location not specified")} {currentEventPhone && `| 📞 ${currentEventPhone}`}</p>
